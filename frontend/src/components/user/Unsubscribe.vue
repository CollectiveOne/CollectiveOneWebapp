...
export default {
  ...
  mounted () {
    if (this.$route.query.fromAll) {
      this.axios.put('/1/secured/notifications/unsubscribeFromAll').then((response) => {
    ...
    if (this.$route.query.fromInitiativeId) {
      this.axios.put('/1/secured/notifications/unsubscribeFromInitiative/' + this.$route.query.fromInitiativeId).then((response) => {
