...
public interface TokenHolderRepositoryIf extends CrudRepository<TokenHolder, UUID> {
	...
	@Query("SELECT holder.tokenType FROM TokenHolder holder WHERE holder.holderId = ?1 AND holder.tokens > 0 ORDER BY holder.tokenType.name ")
	List<TokenType> getTokenTypesHeldBy(UUID holderId);
	
	
	@Query("SELECT holder.tokenType FROM TokenHolder holder WHERE holder.holderId = ?1 AND holder.tokens > 0 AND holder.tokenType.id != ?2")
